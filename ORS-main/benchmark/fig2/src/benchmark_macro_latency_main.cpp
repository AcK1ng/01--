    ...
    for (int i = 0; i < num_samples; i++) {
        ...
        if ((i + 1) % 10 == 0) {
            std::cout << "  Progress: " << (i + 1) << "/" << num_samples 
                      << " samples done. Last sample: " << sample_latency_ms << " ms\n";
        }
    }

    save_results_to_json(output_json_path, "ORS_Framework_Macro", latencies_ms);
}
...
int main(int argc, char** argv) {
    ...
    run_macro_seq_benchmark(master, model_id, move_inputs, 50, 1000, "../results/ors_macro_latency.json");
