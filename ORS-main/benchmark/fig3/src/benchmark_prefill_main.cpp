...
int main(int argc, char** argv) {
    ...
    // 加入了长度 1
    std::vector<int> context_lengths = {1, 4, 16, 32, 64, 128, 512, 1024};
    // 1-128k
    ...
    for (int seq_len : context_lengths) {
        ...
        auto tokens = torch::zeros({1, seq_len}, torch::TensorOptions().dtype(torch::kLong));
        inputs.push_back(tokens);
        
        auto dummy_tensor = torch::empty({0}, torch::TensorOptions().dtype(torch::kFloat));
        ...
        // 1. 预热
        warmup(master, model_id, move_inputs, 50);
        // 2. 正式测速 1000 次
        ...
        std::cout << "SeqLen " << seq_len << " | Mean: " << metrics.mean_ms << "ms | P99: " << metrics.p99_ms << "ms\n";
    }

    mkdir("../results", 0777); 
    save_context_results_to_json("../results/ors_context_latency.json", all_results);
