...
main () {
    ...
    const char *device = "cuda:0";

    torch::Tensor send_t = torch::tensor({1.0, 1.0, 1.0, 1.0}, torch::kFloat);
    send_t = send_t.to(device);
    std::vector<torch::Tensor> send_tmp = {send_t};
    

    torch::Tensor recv_t = torch::tensor({0.0, 0.0, 0.0, 0.0}, torch::kFloat);
    recv_t = recv_t.to(device);
    std::vector<torch::Tensor> recv_tmp = {recv_t};


    {
        std::cout << send_t << std::endl;
        auto work = pg->send(send_tmp, 1, 0);
        work->wait();
    }

    {
        auto work = pg->recv(recv_tmp, 1, 0);
        work->wait();
        std::cout << recv_t << std::endl;
    }
