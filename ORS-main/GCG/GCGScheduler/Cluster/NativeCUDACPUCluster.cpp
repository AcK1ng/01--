            ...
            for_each_leaf__of_two_tuple_tree(*the_payload_from_sender, *ret,
                [&] (c10::IValue &src, c10::IValue &dst) {
                    if (src.isTensor()) {
                        ...
                        torch::cuda::synchronize(acc->GetTorchDevice().index());
...
class NativeCUDACPUAgent: public AcceleratorsAgent {
public:
    NativeCUDACPUAgent(Cluster *cluster) {
...
class NativeCUDACPUCluster final: public Cluster {
public:
    virtual void StartUp() override {

#ifdef WITH_CUDA
        std::cout << "\n====== Starting NativeCUDACPUCluster ======\n";
        ...
        this->accs_agent = std::make_shared<NativeCUDACPUAgent>(this);
        ...
        {
            gpuErrchk(cudaGetDeviceCount(&nr_cudas));
            std::cout << "Detected " << nr_cudas << " CUDA devices\n";
            
            for (int dev = 0; dev < nr_cudas; dev++) {
                cudaDeviceProp cuda_prop;
                gpuErrchk(cudaGetDeviceProperties(&cuda_prop, dev));

                if (last_acc_model != cuda_prop.name) {
                    nr_hosts++;
                    last_acc_model = cuda_prop.name;
                }
...
private:
    std::shared_ptr<NativeCUDACPUAgent> accs_agent;  
};


std::shared_ptr<Cluster>
GetNativeCUDACPUCluster() {
    return std::make_shared<NativeCUDACPUCluster>();
}
