...
class Container(torch.nn.Module):
    def __init__(self, buffers: Tuple[torch.Tensor, ...]):
        super().__init__()
        self.buffers = buffers

def visualize_ts(ts_str, fname):
    ts = torch.parse_ir(ts_str)
    g = Digraph(format="svg")

    source_values__to__gnodes_name = dict()
    values__to__source_values = dict()
    g.node("INPUT")
    for in_value in ts.inputs():
        label = "Input_" + in_value.debugName()
        g.node(label, fillcolor = "blue")

        source_values__to__gnodes_name[in_value] = label
        values__to__source_values[in_value] = in_value
    ...
    for node in ts.nodes():
        if node.kind() == "prim::Constant":
            continue
        elif node.kind() == "prim::TupleIndex":
            input_value = node.inputsAt(0)
            output_value = node.outputsAt(0)
            values__to__source_values[output_value] = values__to__source_values[input_value]
        elif node.kind() == "prim::GCG_Call_Submod":
            submod_name = node.s("target")
            output_value = node.outputsAt(0)
            output_value_lable = submod_name
            source_values__to__gnodes_name[output_value] = output_value_lable
            values__to__source_values[output_value] = output_value

            g.node(output_value_lable)
            for source_value in set([values__to__source_values[value] for value in node.inputs()]):
                source_value_label = source_values__to__gnodes_name[source_value]
                g.edge(source_value_label, output_value_lable)
        else:
            return


    g.node("OUTPUT")
    for out_value in set([values__to__source_values[value] for value in ts.outputs()]):
        label = source_values__to__gnodes_name[out_value]
        g.edge(label, "OUTPUT")
...
def save_to(graph_pack, directory):
    ...
    # push fw_params to container
    scripted = torch.jit.script(Container(fw_params))
    scripted.save(os.path.join(directory, "fw_params_container.pt"))
...
def save_without_states_to(graph_pack, directory):
    ...
    # push fw_buffer/example_input to container
    scripted = torch.jit.script(Container(fw_buffer))
    scripted.save(os.path.join(directory, "fw_buffer_container.pt"))

    scripted = torch.jit.script(Container(example_inputs))
    scripted.save(os.path.join(directory, "example_inputs_container.pt"))
