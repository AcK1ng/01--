...
def create_data_iterator(
    ...
    wikitext_dataset =datasets.load_from_disk(directory + "/dataset/wikitext")
    wikitext_dataset = wikitext_dataset.filter(
        lambda record: record["text"] != "").map(
            lambda record: {"text": record["text"].rstrip("\n")})
    #tokenizer = AutoTokenizer.from_pretrained(tokenizer)
    tokenizer = AutoTokenizer.from_pretrained(directory + "/model_roberta_base/")
...
def train(
    ...
    for step, batch in enumerate(data_iterator, start=start_step):
        ...
        # Forward pass
        loss = model(batch["src_tokens"], batch["attention_mask"], batch["tgt_tokens"])
        # Backward pass
        loss.backward()
        # Optimizer Step
        optimizer.step()
