...
bars = plt.bar(methods, normalized_values, color=colors[:len(normalized_values)], width=0.6)

# 4. 添加数值标注
for bar in bars:
    height = bar.get_height()
    plt.text(bar.get_x() + bar.get_width()/2, height + max(normalized_values)*0.02,
...
plt.ylim(0, max(normalized_values) * 1.25)
plt.tight_layout()
